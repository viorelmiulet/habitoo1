DROP FUNCTION IF EXISTS public.superadmin_delete_organization(uuid);

CREATE OR REPLACE FUNCTION public.superadmin_delete_organization(_org uuid, _actor uuid DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  VALUES (
    NULL, _who, 'organization.hard_deleted', 'organizations', _org,
...
REVOKE ALL ON FUNCTION public.superadmin_delete_organization(uuid, uuid) FROM anon, authenticated, public;
GRANT EXECUTE ON FUNCTION public.superadmin_delete_organization(uuid, uuid) TO service_role;