...
import { propertyStatusLabels, propertyTypeLabels, transactionLabels } from "@/lib/labels";
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        <div className="min-w-0">
          <p className="text-xs text-muted-foreground">
            ...
            {transactionLabels[property.transaction_kind]}
          </p>
          <Link
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-border/70 pt-3 text-xs text-muted-foreground">
            {facts.map((f) => (
              <span key={f.text} className="inline-flex items-center gap-1.5">
                <f.icon className="size-3.5" aria-hidden /> {f.text}
              </span>
            ))}
          </div>
        ) : null}
