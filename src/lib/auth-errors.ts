...
export function classifyAuthError(message?: string | null, code?: string | null): AuthErrorKind {
  ...
  if (m.includes("password") && (m.includes("weak") || m.includes("short") || m.includes("pwned") || m.includes("at least")))
...
const MESSAGES: Record<AuthErrorKind, string> = {
  ...
  email_not_confirmed: "Emailul nu este confirmat încă. Verifică inboxul sau retrimite emailul de confirmare.",
  ...
  provider_disabled: "Autentificarea Google nu este configurată momentan. Folosește emailul și parola.",
