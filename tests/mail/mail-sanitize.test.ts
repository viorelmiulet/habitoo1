...
DOMPurify.addHook("afterSanitizeAttributes", (node) => {
  ...
  if (tag === "A") {
    ...
    if (h && !SAFE_URI.test(h.trim())) el.removeAttribute("href");
  ...
  if (tag === "IMG") {
    ...
    if (s && !SAFE_URI.test(s.trim())) {
      el.removeAttribute("src");
      return;
    }
    if (blockRemoteImages && /^https?:/i.test(s.trim())) {
      el.removeAttribute("src");
      el.setAttribute("data-blocked-src", s);
      blocked++;
    }
  }
});
const san = (html: string, show = false) => {
  blockRemoteImages = !show;
  blocked = 0;
...
test("blochează conținut activ", () => {
  const out = san(
    '<iframe src="x"></iframe><object></object><embed><script>x()</script><p onclick="x()">a</p><a href="javascript:alert(1)">l</a>',
  );
